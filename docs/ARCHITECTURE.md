...
## Предзагрузка шрифтов (sResourceStartUpArchiveList)

Кастомные шрифты в `Interface.ba2` должны быть загружены **при старте игры**, до отрисовки UI.
Игра контролирует это через INI-параметр `sResourceStartUpArchiveList`.

| Платформа | INI | Замена BA2 | Статус |
|-----------|-----|-----------|--------|
| PC | ✓ работает | ✓ работает | Полная поддержка |
| Xbox | ✗ нет доступа | ✓ работает, но откатывается | Частичная |
| PS5 | ✗ вызывает зависание | ✗ нет доступа | Не работает |

**Ключевые моменты:**
- ESM-плагин может регистрировать BA2 для автозагрузки, но неизвестно — startup или on-demand
- Creations (Bethesda) — единственный путь доставки модов на PS5
- **Риск**: если Creations не регистрирует BA2 в startup-списке, кириллица не отрисуется
- **Критический риск PS5**: зависания от INI-модификаций логируются и отправляются в Sony.
  Массовые логи могут быть интерпретированы как попытка эксплойта — это грозит **баном по железу**
...
## Валидация (13 проверок + 1 предупреждение)
...
14. ⚠ Interface BA2: предупреждение о предзагрузке шрифтов (sResourceStartUpArchiveList)
