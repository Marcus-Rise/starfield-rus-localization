...
- [ ] `ba2-packer validate dist/ --source-strings src/strings --source-interface src/interface` — все проверки проходят (профиль `full` для полного кириллического варианта, `standard-font-translit` для translit-варианта)
