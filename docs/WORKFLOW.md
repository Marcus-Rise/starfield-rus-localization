# Workflow: сборка мода из референсного перевода

Пошаговая инструкция по сборке готового мода из существующего перевода (например, v0.42 с Nexus Mods).

## Предварительные требования

- Rust toolchain (`cargo`)
- Файлы референсного перевода (`_ru` нейминг):
...
Референсный `fonts_ru.swf` может содержать проприетарные шрифты. Для публичного релиза нужно пересобрать SWF со свободными шрифтами (SIL OFL):
